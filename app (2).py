...
# -----------------------------------------------------------------------------
# v1.7.0-no-listedness:
# - Removed all Listedness logic and UI (upload, references, calculations, warnings, and column).
# - Kept LLT-PT mapping uploader to enrich event names.
# - All other logic unchanged (parsing, FRD/LRD/TD, validity, reportability, export).
# -----------------------------------------------------------------------------
...
with st.expander("📖 Instructions"):
    st.markdown("""
 - Upload **multiple E2B XML files** and (optional) **LLT-PT mapping Excel** to enrich event names.
...
MG_PATTERN = re.compile(r"\b(\d{1,3}(?:,\d{3})*|\d+)(?:\.\d{1,3})?\s*mg\b", re.IGNORECASE)
...
# --------------------- GLOBAL FRD/LRD/TD (document order) --------------------
